...
def tournaments(net1, net2, player, first_player, winning, boards, MCTS=False):
    for board in boards:
        if player == 1:
            if MCTS:
                black_player = play.MCTSPlayer(net=net1, simulations_per_move=800, th=0)
            ...
            if net2 == None:
                ...
                white_player = play.MCTSPlayer(net=net2, simulations_per_move=800, th=0)
            ...
            if MCTS:
                white_player = play.MCTSPlayer(net=net1, simulations_per_move=800, th=0)
                white_player.initialize_game(board=board, player=first_player)
            else:
            ...
            if net2 == None:
                ...
                black_player = play.MCTSPlayer(net=net2, simulations_per_move=800, th=0)
                black_player.initialize_game(board=board, player=first_player)
            else:
...
if __name__ == '__main__':
    ...
    MCTS = True
    models1 = [ './modelNonPC/model149.model']
    models2 = ['./model/model149.model']
    for idx in range(len(models1)):
        ...
        with multiprocessing.Manager() as mg:
            ...
            jobs = [Process(target=tournaments, args=(net1[i], net2[i], 1, 1, winning, [black_board[i]], MCTS)) for i in range(50)]
            for j in jobs:
                j.start()
            for j in jobs:
                j.join()
            ...
            for j in jobs:
                j.join()
            black_white_score = 50 - np.sum(winning)
            print(black_white_score)
            winning = multiprocessing.Manager().list([])
            jobs = [Process(target=tournaments, args=(net1[i], net2[i], 1, -1, winning, [white_board[i]], MCTS)) for i in range(50)]
            for j in jobs:
                j.start()
            for j in jobs:
                j.join()
            ...
            for j in jobs:
                j.join()
            white_white_score = 50 - np.sum(winning)
        fr = open('tournamentPK.txt', 'a')
        fr.write(model1 + '\t' + model2 + '\t' + str(black_black_score) + '\t' + str(black_white_score) + '\t' + str(white_black_score) + '\t' + str(white_white_score) + '\t' + str(black_black_score + black_white_score + white_black_score + white_white_score) + '\n')
        fr.close()
