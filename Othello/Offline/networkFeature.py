...
class Network(nn.Module):
    def __init__(self, channel=256, numBlock=3):
        ...
        self.policy_conv = nn.Sequential(
            nn.Conv2d(in_channels=self.channel, out_channels=2, kernel_size=1, stride=1, padding=0),
            nn.BatchNorm2d(2),
            nn.ReLU()
        )
    ...
    def forward(self, feature):
        ...
        policy = policy.view(-1, 128)
        policy = self.policy_fc(policy)
        value = self.value_conv(feature)
        valueConv = value.view(-1, 64)
...
class PV():
    ...
    def run(self, board):
        probs, values, valueConv = self.run_many([board])
        return probs[0], values[0], valueConv[0]

    def run_many(self, boards):
        fs = np.array(boards, dtype=np.float32)
        fs = torch.FloatTensor(fs)
        log_act_probs, value, valueConv = self.net(Variable(fs.cuda(self.num)))
        act_probs = np.exp(log_act_probs.data.cpu().numpy())
        value = value.data.cpu().numpy()
        valueConv = valueConv.data.cpu().numpy()
        return act_probs, value, valueConv
