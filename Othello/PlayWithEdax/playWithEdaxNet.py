...
if __name__ == '__main__':
    ...
    parallel = 10
    interval = 50
    black_board = load('black.txt')
    white_board = load('white.txt')
    models = ['./modelMCTS/model49.model', './modelMCTS/model99.model', './modelMCTS/model149.model', './modelMCTS/model199.model', './modelMCTS/model249.model', './modelMCTS/model299.model', './modelMCTS/model349.model', './modelMCTS/model399.model']
    for sims in [800]:
        for model in models:
            net = []
            ...
            with multiprocessing.Manager() as mg:
                ...
                jobs = [Process(target=tournaments, args=(net[i], 1, 1, winning, black_board[i * interval : (i + 1) * interval], sims, False)) for i in range(parallel)]
                for j in jobs:
                    j.start()
                for j in jobs:
                    j.join()
                ...
                winning = multiprocessing.Manager().list([])
                jobs = [Process(target=tournaments, args=(net[i], 1, -1, winning, white_board[i * interval : (i + 1) * interval], sims, False)) for i in range(parallel)]
                for j in jobs:
                    j.start()
                for j in jobs:
                    j.join()
