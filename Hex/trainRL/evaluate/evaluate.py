...
def single_play(net1, net2, game, simulation, old_winning, flag):
    ...
    for i in range(args.game_num // parallel_num):
        ...
        opening = random.sample([i for i in range(169)], 1)[0]
        player1.make_move(opening // hex.SIZE, opening % hex.SIZE)
        player2.make_move(opening // hex.SIZE, opening % hex.SIZE)
        color = hex.WHITE
...
if __name__ == '__main__':
    ...
    modelNames = []
    pgnName = 'model_10_128_PC.pgn'
    fileName = 'elo_10_128_PC.txt'
    for modelIndex in range(len(modelNames)-1):
        starttime = time.time()
        parallel = args.parallel_num
        pc1name = './modelPC_' + lam + '/' + modelNames[modelIndex]
        pc2name = './modelPC_' + lam + '/' + modelNames[modelIndex+1]
    nets1 = []
    nets2 = []
    for i in range(parallel):
        nets1.append(network.PV(model_path=pc1name, channel=128, numBlock=10, num=i%torch.cuda.device_count()))
        nets2.append(network.PV(model_path=pc2name, channel=128, numBlock=10, num=i%torch.cuda.device_count()))
    ...
    old_name = pc1name
    write_pgn(old_name, pc2name, whitewins, blackwins, pgnName)

    line = pc1name + ' vs ' + pc2name + ' = ' + str(old) + ':' + str(2 * game_num - old)
    f = open(fileName, 'a')
    f.write(line + '\n')
    f.close()
    endtime = time.time()
