...
parser.add_argument('--parallel_num', default=12, type=int)
...
def single_play(net1, net2, start, end, simulation, old_winning, flag):
    ...
    for i in range(start, end):
        print("Game:", i)
        if simulation == 0:
            player1 = GreedyPlayer(net=net1)
            player2 = GreedyPlayer(net=net2)
        else:
...
def write_pgn(old_name, new_name, new_white, new_black, record='wbec.pgn'):
    ...
    white = old_name
    black = new_name
    whitewins = hex.SIZE**2 - new_black  # old model is white
    for _ in range(hex.SIZE**2):
        f.write("[Event \"F/S\"]\n")
...
if __name__ == '__main__':
    ...
    simulations = [0, 800, 1600]
    boardSizes = [13]
    models = ['PC_value_13.model', 'PC_feaure_13.model', 'PC_mix_13.model']
    for simulation in simulations:
        for h in boardSizes:
            for model in models:
                starttime = time.time()
                parallel = args.parallel_num
                pc1name = 'NonPC_13.model'
                pc2name = model
                nets1 = []
                nets2 = []
                for i in range(parallel):
                    nets1.append(networkReLU3.PV(model_path=pc1name, num=i%torch.cuda.device_count()))
                    nets2.append(networkReLU3.PV(model_path=pc2name, num=i%torch.cuda.device_count()))
                l = []
                old = 0
                whitewins = 0
                blackwins = 0
                ...
                with multiprocessing.Manager() as mg:
                    ...
                    interval = hex.SIZE * hex.SIZE // parallel
                    jobs = [Process(target=single_play,
                            args=(nets1[i], nets2[i], interval*i, interval*(i+1), simulation, winning_list, flag)) for i in range(parallel-1)]
                    jobs.append(Process(target=single_play, args=(nets1[i], nets2[i], interval*i, hex.SIZE * hex.SIZE, simulation, winning_list, flag)))
                    for j in jobs:
                        j.start()
                    for j in jobs:
                        j.join()

                    whitewins = hex.SIZE**2 - len(winning_list)
                    flag = -flag
                    jobs = [Process(target=single_play,
                            args=(nets1[i], nets2[i], interval*i, interval*(i+1), simulation, winning_list, flag)) for i in range(parallel-1)]
                    jobs.append(Process(target=single_play, args=(nets1[i], nets2[i], interval*i, hex.SIZE * hex.SIZE, simulation, winning_list, flag)))
                    ...
                    blackwins = 2 * hex.SIZE**2 - old - whitewins
                    print("old wins", len(winning_list))

                old_name = pc1name
                write_pgn(old_name, pc2name, whitewins, blackwins, 'modelPKNet.pgn')

                line = str(simulation) + '\t' + pc1name + ' vs ' + pc2name + ' = ' + str(old) + ':' + str(2 * hex.SIZE**2 - old) + '\t' + str(whitewins) + '\t' + str(blackwins)
                f = open('eloNet.txt', 'a')
                f.write(line + '\n')
                f.close()
                endtime = time.time()
