...
class Network(nn.Module):
    def __init__(self, channel=128, numBlock=3):
    ...
    def forward(self, feature):
        ...
        policy = policy.view(-1, 13 * 13)
        policy = self.policy_fc(policy)
        value = self.value_conv(feature)
        value = value.view(-1, 13 * 13)
        value = self.value_fc(value)
        return policy, value, feature


class PV():
    def __init__(self, model_path=None, channel=128, numBlock=3, num=0):
