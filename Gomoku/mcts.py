...
class MCTSNode(object):
    ...
    def __init__(self, board, fmove=None, parent=None, cpuct=1.5):
        ...
        self.child_N = np.zeros([SIZE * SIZE], dtype=np.float32)
        self.child_W = np.zeros([SIZE * SIZE], dtype=np.float32)

        self.original_prior = np.zeros([SIZE * SIZE], dtype=np.float32)
        self.child_prior = np.zeros([SIZE * SIZE], dtype=np.float32)
        self.children = {}
    ...
    def maybe_add_child(self, action):
        if action not in self.children:
            new_position = self.board.play_move(action // SIZE, action % SIZE)
    ...
    def incorporate_results(self, move_probabilities, value, up_to):
        assert move_probabilities.shape == (SIZE * SIZE,)
    ...
    def inject_noise(self):
        dirch = np.random.dirichlet([0.3] * (SIZE * SIZE))
